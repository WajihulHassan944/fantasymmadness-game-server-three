  ...
  if (fighterAImageUrl && fighterBImageUrl) {
    ...
    if (req.files.promotionBackground && !promotionBackground) {
      ...
      const responseBackground = await fetch(
        'https://api.imgbb.com/1/upload?key=368cbdb895c5bed277d50d216adbfa52',
        {
          method: 'POST',
          body: formDataBackground,
        }
...
cron.schedule('* * * * *', async () => {
  console.log('Cron job started.');
>>>>>>> 90ff05c3fd0fd6889f259dc14ec46210a4c937de
  try {
    const now = new Date();
...
app.get('/api/cron-job', async (req, res) => {
  ...
  try {
    ...
    for (const match of matchesToConvert) {
      ...
      console.log(`Converted match ${match._id} to shadow.`);
      await Match.findByIdAndUpdate(match._id, { matchType: 'SHADOW' });
