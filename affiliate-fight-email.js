...
const renderMessage = (message) => String(message || '').split(/(https:\/\/[^\s<>]+)/g)
  .map((part) => /^https:\/\//.test(part)
    ? `<a href="${escapeHtml(part)}" style="color:#ffcb57;word-break:break-all">${escapeHtml(part)}</a>`
    : escapeHtml(part).replace(/\n/g, '<br>'))
  .join('');

function affiliateFightEmail({ affiliate, fight, fightId, appUrl, message, posterCid }) {
...
${poster ? `<tr><td align="center" style="padding:10px 14px"><img src="${poster}" alt="${fighterA} versus ${fighterB} fight poster" width="550" style="display:block;width:100%;max-width:550px;height:auto"></td></tr>` : `<tr><td style="padding:10px 14px"><table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr>
...
<tr><td align="center" style="padding:4px 22px 14px"><a href="${safeImage(fight.fightPosterImage || fight.promotionBackground)}" style="color:#ffcb57;font-size:14px;word-break:break-all">Open the full fight poster</a></td></tr>
<tr><td style="padding:18px 24px;color:#f3f5fb;font-size:14px;line-height:1.65;text-align:left"><strong style="color:#ffcb57">YOUR FIGHT INSTRUCTIONS AND READY-TO-POST CAPTIONS</strong><div style="padding-top:12px">${renderMessage(message)}</div></td></tr>
